...
// =============================================================================
// INTENT MANIFOLD + GAUGE CHECKING LAYER (v0.3 First Piece)
// =============================================================================

/// Represents a point on the Intent Manifold.
/// This is the structured representation of user intent.
/// Gauge transformations (synthesis steps) must preserve these invariants.
#[derive(Clone, Debug)]
struct Intent {
    ...
    ritual_like: bool,
    adaptive: bool,
    // Future: more fields for richer manifold
}
...
impl Intent {
    fn from_task(task: &Task) -> Self {
        Self {
            task_type: task.name,
            requires_safety: task.features[3] > 0.5,
            requires_progress: task.features[4] > 0.5,
            requires_observations: task.features[1] > 0.5 || task.features[2] > 0.5,
            num_tactics: (task.features[0] * 8.0) as u8,
            ritual_like: task.features[5] > 0.4,
            adaptive: task.features[6] > 0.4,
        }
    }
}

/// Gauge invariants that must be preserved during synthesis.
/// These are the "fibers" in the gauge-theoretic model.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum GaugeInvariant {
    SafetyExactlyOneActive,
    ProgressTimerDriven,
    RitualPhasesTerminate,
    ExhaustiveCoverage,
    ObservationsUsed,
}
...
impl GaugeInvariant {
    fn description(&self) -> &'static str {
        match self {
            GaugeInvariant::SafetyExactlyOneActive => "SAFETY: exactly_one_active_tactic",
            GaugeInvariant::ProgressTimerDriven => "PROGRESS: timer_driven_evaluation",
            GaugeInvariant::RitualPhasesTerminate => "RITUAL: phases_are_linear_and_terminate",
            GaugeInvariant::ExhaustiveCoverage => "COVERAGE: exhaustive_match",
            GaugeInvariant::ObservationsUsed => "OBSERVATION: health_ratio_used",
        }
    }
}
...
impl SynthesisState {
    ...
    /// Check if the current state satisfies the gauge invariants required by the intent
    fn satisfies_gauge(&self, intent: &Intent) -> bool {
        let has_safety = self.invariants.iter().any(|i| *i == Some(GaugeInvariant::SafetyExactlyOneActive.description()));
        let has_progress = self.invariants.iter().any(|i| *i == Some(GaugeInvariant::ProgressTimerDriven.description()));
        let has_coverage = self.invariants.iter().any(|i| *i == Some(GaugeInvariant::ExhaustiveCoverage.description()));

        has_safety && (has_progress || !intent.requires_progress) && has_coverage
    }
}
...
// =============================================================================
fn synthesize(task: &Task, intent: &Intent) -> Option<(SynthesisState, Vec<usize>)> {
    ...
    // v0.3: Also enforce gauge invariants from the Intent Manifold
    if verify(&state, task) && state.satisfies_gauge(intent) {
        Some((state, applied))
    } else {
        None
    }
...
fn emit_certified_code(task: &Task, state: &SynthesisState, _applied: &[usize], intent: &Intent) -> String {
    let mut code = String::with_capacity(MAX_CODE_BUFFER);

    // Header
    writeln!(&mut code, "// ═══════════════════════════════════════════════════════════════════════════").unwrap();
    writeln!(&mut code, "// GaugeForge Certified Output — v0.3 Intent Manifold + Gauge Layer").unwrap();
    writeln!(&mut code, "// Task: {} | Gauge invariants preserved: {}", task.name, intent.task_type).unwrap();
    writeln!(&mut code, "// This code was synthesized under gauge constraints. All invariants verified.").unwrap();
    writeln!(&mut code, "// ═══════════════════════════════════════════════════════════════════════════\n").unwrap();
    ...
    for i in 0..state.num_tactics {
        if let Some(t) = state.tactics[i] {
            ...
            if i < state.num_tactics - 1 { write!(&mut code, ",").unwrap(); }
        }
    }
    writeln!(&mut code, " }}\n").unwrap();
    ...
    let first_tactic = if state.num_tactics > 0 { state.tactics[0].unwrap_or("Adaptive") } else { "Adaptive" };
    writeln!(&mut code, "        Self {{ current: Tactic::{}, tick_count: 0 }}", first_tactic).unwrap();
    writeln!(&mut code, "    }}\n").unwrap();

    // Tick method - CLEAN structure
    writeln!(&mut code, "    /// Synthesized under gauge constraints from Intent Manifold").unwrap();
    writeln!(&mut code, "    pub fn tick(&mut self, health_ratio: f32, player_count: u32) {{").unwrap();
    writeln!(&mut code, "        self.tick_count = self.tick_count.wrapping_add(1);\n").unwrap();

    // Progress timer (if required by intent)
    if intent.requires_progress {
        writeln!(&mut code, "        // PROGRESS invariant: timer-driven evaluation").unwrap();
        writeln!(&mut code, "        if self.tick_count % 4 == 0 {{").unwrap();
        writeln!(&mut code, "            let new_tactic = match (health_ratio, player_count) {{").unwrap();
        writeln!(&mut code, "                // Gauge-verified arms from synthesis").unwrap();
        writeln!(&mut code, "                (h, _) if h < 0.35 => Tactic::Defensive,").unwrap();
        writeln!(&mut code, "                (_, p) if p >= 3 => Tactic::Adaptive,").unwrap();
        ...
        writeln!(&mut code, "        // No progress timer required by current intent").unwrap();
    }
    ...
    writeln!(&mut code, "// GaugeForge Certified — Intent preserved, invariants verified.").unwrap();
    code
}
...
fn main() {
    ...
    let intent = Intent::from_task(&task);

    match synthesize(&task, &intent) {
        Some((state, applied)) => {
            let trace = render_trace(&task, &applied, &state);
            println!("{}", trace);

            // v0.3: Pass Intent for gauge-aware emission
            let code = emit_certified_code(&task, &state, &applied, &intent);
            println!("\n════════════════════════════════════════════════════════════════════════════");
            println!("CERTIFIED OUTPUT (GaugeForge v0.3 — Intent Manifold + Gauge Layer)");
            println!("════════════════════════════════════════════════════════════════════════════\n");
            println!("{}", code);

            println!("\n[Scotty] Synthesis complete. Gauge invariants preserved. No hallucinations.");
